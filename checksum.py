...
CK_ENABLED  = 0x30
CK_DISABLED = 0xFF
CAL_BASE_FULL = 0x14000
CAL_CRC_DISABLE_BIT = 0x10
...
def program_check_disabled(data) -> bool:
    """True when the full-ROM program checksum gate is explicitly disabled."""
    return len(data) == FULL_ROM_SIZE and data[CHECKSUM_SWITCH_ADDR] == CK_DISABLED


def cal_check_disabled(data) -> bool:
    """True when calibration CRC verification is disabled in the cal control bytes."""
    base = CAL_BASE_FULL if len(data) == FULL_ROM_SIZE else (0 if len(data) == TUNE_SIZE else None)
    return bool(base is not None and base + 8 <= len(data)
                and data[base + 7] & CAL_CRC_DISABLE_BIT)


def checksum_status(data) -> dict:
    """Structured status used by the in-process flash safety gates."""
    d = bytearray(data)
    if len(d) == FULL_ROM_SIZE:
        cal_ok, _, cal_count = _cal_verify(d)
        return {
            "boot": _boot_calc(d) == _u16le(d, _BOOT_STORE),
            "program": _prog_calc(d) == _u16le(d, _PROG_STORE),
            "cal": cal_ok if cal_count else None,
            "prog_disabled": program_check_disabled(d),
            "cal_disabled": cal_check_disabled(d),
        }
    if len(d) == TUNE_SIZE:
        cal_ok, _, cal_count = _cal_verify(d)
        return {
            "boot": None, "program": None, "cal": cal_ok if cal_count else None,
...
def verify_checksum(data: bytearray) -> Tuple[bool, List[str]]:
    ...
    if size == FULL_ROM_SIZE:
        ...
        if pc == ps:
            ...
            prog_result = "MISMATCH (IGNORED — PROGRAM CHECK DISABLED)"
        else:
            prog_result = "MISMATCH"
        details.append(f"Program      : stored 0x{ps:04X} / calc 0x{pc:04X}  "
                       f"{prog_result}")
        # Stock MS41 firmware does not enforce this value when the gate at
        # 0x605C is 0xFF.  This is the normal MS41.3 case: boot and calibration
        # still have to pass, but a program-checksum mismatch is informational.
        ok &= (pc == ps or prog_disabled)
