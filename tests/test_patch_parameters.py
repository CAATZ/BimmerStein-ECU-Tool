...
CASES = (
    (
        "MS41.0",
        ["ignition_cut_v9_ms410", "launch_control_v7_ms410", "vanos_minrpm_v2_ms410"],
        {"ignition_cut_v9_ms410": 4, "launch_control_v7_ms410": 10,
         "vanos_minrpm_v2_ms410": 1},
    ),
    (
        "MS41.1",
        ["ignition_cut_v9_ms411", "launch_control_v7_ms411"],
        {"ignition_cut_v9_ms411": 4, "launch_control_v7_ms411": 10},
    ),
    (
        "MS41.2",
        ["ignition_cut_v9_ms412", "launch_control_v7_ms412"],
        {"ignition_cut_v9_ms412": 4, "launch_control_v7_ms412": 10},
    ),
    (
        "MS41.3",
        ["ignition_cut_v9", "launch_control_v7"],
        {"ignition_cut_v9": 4, "launch_control_v7": 10},
    ),
)

EXPECTED_PARAMETER_IDS = {
    "ignition_cut_v9": {"CUTSW", "CUTRPM", "CUT_HYST", "CUT_IPW"},
    "launch_control_v7": {
...
def test_ignition_parameters_round_trip_without_touching_identity(variant, patch_id):
    ...
    result, report = patch_service.apply_parameter_changes(
        source,
        patch_id,
        {"CUTSW": "pin80", "CUTRPM": "4000", "CUT_HYST": "64", "CUT_IPW": "0"},
...
def test_launch_parameters_validate_and_keep_named_sentinels():
    source = _built("MS41.3", ["ignition_cut_v9", "launch_control_v7"])
    groups = {group["patch_id"]: group for group in patch_service.editable_parameters(source)}
    launch = groups["launch_control_v7"]
    ...
    values = {
        ...
        if group["patch_id"] == "launch_control_v7"
...
def test_unknown_stale_partial_and_uninstalled_requests_are_rejected():
    source = _built("MS41.3", ["ignition_cut_v9"])
    with pytest.raises(patch_service.PatchError, match="unknown patch parameter"):
        patch_service.apply_parameter_changes(source, "ignition_cut_v9", {"OFFSET": "1"})
    with pytest.raises(patch_service.PatchError, match="string ids and values"):
        patch_service.apply_parameter_changes(source, "ignition_cut_v9", {"CUTRPM": 4000})
    with pytest.raises(patch_service.PatchError, match="source ROM changed"):
        patch_service.apply_parameter_changes(
            source, "ignition_cut_v9", {"CUTRPM": "4000"}, expected_sha256="0" * 64)
    ...
    with pytest.raises(patch_service.PatchError, match="not an editable current installation"):
        patch_service.apply_parameter_changes(
            ref("MS41.3"), "ignition_cut_v9", {"CUTRPM": "4000"})
