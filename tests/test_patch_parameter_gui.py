...
def test_installed_parameter_patch_has_configure_button():
    ...
    try:
        image = patch_service.build_image(ref("MS41.3"), ["ignition_cut_v7"])[0]
        window._set_patch_base(image, "parameter-test.bin")

        group = window._patch_parameter_groups["ignition_cut_v7"]
        row = window._patch_rows["ignition_cut_v7"]
