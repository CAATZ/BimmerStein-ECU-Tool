...
def test_msvc_runtime_verifier_rejects_modified_dependency_copy(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {Path("VCRUNTIME140.dll"): (source, "test dependency")},
    )
...
def test_release_packaging_requires_explicit_license_gates():
    ...
    assert '$releaseName = "BimmerStein-ECU-Tool-$Version-Windows-x64"' in text
...
def test_readme_uses_canonical_product_logo_and_beta_links():
    text = (ROOT / "README.md").read_text(encoding="utf-8")
    assert '<img src="assets/bimmerstein_ecu_tool.png"' in text
    assert 'alt="BimmerStein ECU Tool"' in text
    assert 'href="RELEASE_NOTES.md">Beta Release Notes</a>' in text
