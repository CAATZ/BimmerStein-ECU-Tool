...
ALPHAN_FIXED_PATH = (
    TEST_DATA_ROOT / "REF_MS41.3" / "MS41.3_s52_alphan_failsafe_fixed.bin")
_REFERENCE_VARIANTS = {
...
def verify_calguard_compatibility():
    """Execute exact installed V4 bytes on every canonical family and failures."""
    ...
    result = _run(emu,
        hook_cpu, stop_at=(BOOT_EXIT, RECOVER_EXIT),
        max_steps=POLL_COUNT * 5 + 5000)
    assert result.final_ip == BOOT_EXIT and CAVE_CPU in visited
    ...
    assert _verify_calguard(
        installed["MS41.2"][0], variant=installed["MS41.2"][1],
        e740=1) == "RECOVER"
    ...
    emu.write_byte(0xE740, 3)
    assert _run(emu,
        CAVE_CPU, breakpoints=(poll_loop,), max_steps=1000).final_ip == poll_loop
    for index, byte in enumerate(RECOVERY_TOKEN):
        emu.asc0.rx_inject(bytes([byte]))
        emu.write_byte(0xFF6E, emu.read_byte(0xFF6E) | 0x80)
        assert _run(emu,
            poll_loop, breakpoints=(poll_byte,), max_steps=10).final_ip == poll_byte
...
def verify_alphan_failsafe():
    ...
    fixed = ALPHAN_FIXED_PATH.read_bytes()
    assert hashlib.sha256(fixed).hexdigest() == (
        "742ccabc12bc186cd463121cc46ecf6d63a65e21d341fc01aff3c386be709786")
    assert patch_ms41.is_applied(FULL_413_IMAGE, patch)
    for edit in patch["edits"]:
        off = edit["off"]
        data = bytes.fromhex(edit["data"])
        assert fixed[off:off + len(data)] == data
        assert FULL_413_IMAGE[off:off + len(data)] == data

    def run(selector, fault):
        emu = _emu(FULL_413_IMAGE, dpp0=4)
        emu.reg.set_word(4, 0x1234)
        emu.write(0xFD22, selector)
        emu.write(0xFD30, fault)
        emu.write_byte(0xFC3C, 22)
        emu.write_byte(0xE8D0, 3)
        before = emu.state()["regs"]
        visited = []
        emu.cpu.set_trace(lambda pc, _opcode: visited.append(pc))
        result = _run(emu, 0x2E9A8, stop_at=0x2E9AE, max_steps=1500)
        assert (
            result.exit_reason == "stop_at"
            and result.final_pc == 0x2E9AE
            and result.regs["sp"] == before["sp"]
            and result.regs["dpp"] == before["dpp"]
        ), result
        return emu, set(visited)

    normal, normal_path = run(0, 0)
    assert (
        0x3DB6A in normal_path
        and 0x3DB7A not in normal_path
        and normal.read(0xE8E4) == 0x1234
    )
    _selector, selector_path = run(0x0400, 0)
    assert (
        0x3DB7A in selector_path
        and 0x3DB6E not in selector_path
        and 0x3DBC0 in selector_path
    )
    _fault, fault_path = run(0, 0x0002)
    assert 0x3DB6E in fault_path and 0x3DB7A in fault_path
...
def verify_loader_and_doors():
    for version, case in SOFTBSL_VARIANTS.items():
        ...
        emu = _emu(image)
        res = _run(emu, 0x15A0, stop_at=(0x1D92,), max_steps=10)
        assert res.final_ip == 0x1D92 and res.exit_reason == "stop_at", (version, res)

        emu = _emu(image)
        emu.write_byte(0xE653, 0x00)
        res = _run(emu, 0x1D92, stop_at=(0x0A44,), max_steps=100)
        ...
        res = _run(emu, 0x1D92, stop_at=(0x1FD8,), max_steps=100)
        assert (
            res.final_ip == 0x1FD8
            and res.exit_reason == "stop_at"
        ...
        # V10 rejects empty and oversized agents before ACK, receive, RAM copy,
        ...
        for upload in (
            ...
            res = _run(emu, 0x1D92, stop_at=(0xD800,), max_steps=300000)
...
def _admission_fingerprint():
    ...
    add("alphan/fixed.bin", ALPHAN_FIXED_PATH)
    asm = TEST_DATA_ROOT.parent / "Decompilation" / "asm"
...
def _group_features_413():
    checks = [
        ("AlphaN V2 fallback", verify_alphan_failsafe),
